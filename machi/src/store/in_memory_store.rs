...
impl<D: Serialize + Eq> InMemoryVectorStore<D> {
    ...
    pub fn from_documents(documents: impl IntoIterator<Item = (D, OneOrMany<Embedding>)>) -> Self {
        ...
        documents
            ...
            .for_each(|(i, (doc, embedding))| {
                store.insert(format!("doc{i}"), (doc, embedding));
            });
    ...
    pub fn from_documents_with_ids(
        ...
        let mut store = HashMap::new();
        documents.into_iter().for_each(|(i, doc, embedding)| {
            store.insert(i.to_string(), (doc, embedding));
        });
    ...
    pub fn from_documents_with_id_f(
        ...
        documents.into_iter().for_each(|(doc, embedding)| {
            store.insert(f(&doc), (doc, embedding));
        });
    ...
    fn vector_search_lsh(
        ...
        for candidate_id in candidates {
            if let Some((doc, embedding)) = self.embeddings.get(&candidate_id) {
                // Get the best context for the document given the prompt
                if let Some((distance, embed_doc)) = embedding
                    .iter()
                    .map(|embedding| {
